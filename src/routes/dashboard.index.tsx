...
function Overview() {
  ...
  const monthlyHolding = enriched.reduce((acc, s) => acc + safeNum(s.stock) * safeNum(s.unit_cost) * 0.02, 0);
  ...
  const urgent = enriched
    ...
    .sort((a, b) => b.opt.recommendedOrder * safeNum(b.unit_cost) - a.opt.recommendedOrder * safeNum(a.unit_cost))
    .slice(0, 3);
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 max-w-7xl mx-auto">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 mb-6">
        <KpiCard label="Valeur totale du stock" value={`${inventoryValue.toLocaleString("fr-FR", { maximumFractionDigits: 0 })} €`} sub="Valorisation au coût d'achat" icon={DollarSign} tone="default" to={{ to: "/dashboard/skus" as const, search: {} }} loading={loading} />
        <KpiCard label="Risques ruptures (Urgent)" value={`${critical} SKUs`} sub="Stock < 50% de sécurité" icon={AlertTriangle} tone="danger" to={{ to: "/dashboard/skus" as const, search: { status: "critical" } }} loading={loading} />
        <KpiCard label="Commandes recommandées" value={`${toReorder} SKUs`} sub="Point de commande dépassé" icon={Boxes} tone="warning" to={{ to: "/dashboard/solver" as const, search: {} }} loading={loading} />
        <KpiCard label="Commandes en cours (PO)" value="0 active" sub="En attente ou transit" icon={Truck} tone="info" to={{ to: "/dashboard/pos" as const, search: {} }} loading={loading} />
      </div>
                ...
                <div key={s.id} className="flex items-center justify-between gap-3 p-3 rounded-xl border border-border bg-background/50">
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="text-[11px] text-muted-foreground font-mono">
                      Stock: {safeNum(s.stock)} u | ROP: {s.opt.reorderPoint} u | Invest: {(s.opt.recommendedOrder * safeNum(s.unit_cost)).toLocaleString("fr-FR", { maximumFractionDigits: 0 })} €
                    </div>
                  </div>
...
function KpiCard({
  ...
  label: string; value: string; sub: string; icon: typeof Boxes;
  tone: "default" | "danger" | "warning" | "info";
  to: any; loading: boolean;
}) {
  const ring = tone === "danger" ? "border-destructive/60" : tone === "warning" ? "border-warning/60" : tone === "info" ? "border-primary/40" : "border-border";
  const iconColor = tone === "danger" ? "text-destructive" : tone === "warning" ? "text-warning" : "text-primary";
  return (
    <Link {...to} className={`block rounded-2xl border ${ring} bg-card p-4 hover:border-primary/60 hover:shadow-[var(--shadow-elegant)] transition-all`}>
...
function LegendItem({ color, label, pct }: { color: string; label: string; pct: number }) {
  const bg = color === "success" ? "bg-success" : color === "warning" ? "bg-warning" : color === "destructive" ? "bg-destructive" : "bg-chart-5";
  return (
